...
def validar_datos(datos):
    ...
    aa, ap = datos["anio_actual"], datos["anio_anterior"]
    ...
    if all(v is None for v in datos["sla_prom_por_anio"].values()):
        errores.append("ERROR CRÍTICO: Todos los SLA son None")
    meses = list(datos["resumen_mes"].keys())
    if not any(len(m) == 7 and "-" in m for m in meses):
...
def main():
    ...
    print(f"Auto:     pre={datos['automatizacion']['pre']['total']} post={datos['automatizacion']['post']['total']}")
    print("=== FIN DEBUG ===\n")
